...
def draw_parallel_lines(n, point, length, window):
    ...
    line = rg.Line(rg.Point(point.x, point.y + 10), rg.Point((point.x + length), point.y + 10))
    line.attach_to(window)
    """
    for k in range(n):
...
def draw_lines(n, point, window):
    ...
    Type hints:
      ...
      :type window: rg.RoseWindow
    """
    separation = ((point.y + 100) - (point.y - 100)) / (n - 1)
    print(separation)
    for k in range(n):
        line = rg.Line(point, rg.Point(point.x + 100, point.y - 100 + separation * k))
        line.attach_to(window)
